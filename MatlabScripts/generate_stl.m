...
function generate_stl(inputData,sizeOfFinalLattice,ID,resolutionOfUnitCell,resolutionOfFinalLattice,fillFractionTarget)
    createFolder('MatFiles')
    
    resolutionOfUnitCell = resolutionOfUnitCell * [1,1,1];
    resolutionOfFinalLattice = resolutionOfFinalLattice * [1,1,1];
    ...
    jointRadius = 10; %Set initial joint Radius
    ...
    for i = 1:length(IDNumberList)
        ...
        %Loop to find Fill Fraction
            ...
            while abs(whileVarFillFraction)>.0001 && count<countLimit && switchCountVar < switchLimit
                ...
                jointRadius = jointRadius + whileAdjustmentValue*switchVar; %Adjust joint radius by the adjustment value. SwitchVar gives direction that you are going.
                if jointRadius<.5 %If jointRadius would round to zero or below, make it round to 1. 0 value screws things up.
                    jointRadius = .5;
                end
                Distances = GetDistanceValues(UCTypeList(i),strutRadiusList(i,:),jointRadius); 
                finalFillFraction = CheckPreviousCalculations(UCTypeList(i), resolutionOfUnitCell, strutRadiusList(i,:), jointRadius, fillFractionResultsMatrix);
        ...
        latticeStats(IDNumberList(i),:)=[IDNumberList(i),sizeOfFinalLattice,resolutionOfUnitCell,resolutionOfFinalLattice,fillFractionTarget,finalFillFraction,MeshSurfaceMax,MeshVolMax,UCTypeList(i),replicationList(i,:), strutRadiusList(i,:), jointRadius, strutStretchList(i),ZAdjustToFillFractionList(i)];
